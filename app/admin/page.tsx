...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold font-label uppercase tracking-wide ${c.bg} ${c.text}`}
    >
...
function StatCard({
  ...
  return (
    ...
    >
      <div
        className={`w-9 h-9 rounded-xl flex items-center justify-center mb-4 ${accent ? "bg-primary/15 text-primary" : "bg-black/[0.04] text-[#1A1714]/40"}`}
      >
        {icon}
      </div>
      <p className="text-2xl font-bold text-[#1A1714] font-headline mb-0.5">
        {value}
      </p>
      <p className="text-xs text-[#1A1714]/40 font-label">{label}</p>
      {sub && (
        <p className="text-[11px] text-emerald-600 font-label mt-1 flex items-center gap-1">
...
export default function AdminDashboardPage() {
  ...
  const fetchAll = async () => {
    ...
    const resolvedToday =
      todayData ?? allOrders.filter((o) => o.created_at >= todayIso);
    const resolvedMonth =
      totalData ?? allOrders.filter((o) => o.created_at >= monthIso);

    setStats({
      todayOrders: resolvedToday.length,
      todayRevenue: resolvedToday.reduce((s, o) => s + o.total, 0),
      pendingDispatch: resolvedToday.filter((o) => o.status === "confirmed")
        .length,
  ...
  return (
    <div ref={pageRef}>
      ...
      <div className="dash-card flex items-center justify-between mb-8">
        <div>
          <p className="text-[11px] uppercase tracking-[0.2em] text-[#1A1714]/30 font-label mb-1">
            Overview
          </p>
          <h1 className="text-2xl font-bold text-[#1A1714] font-label tracking-tight">
            Dashboard
          </h1>
        </div>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard
          ...
          value={
            loading ? "—" : `NPR ${(stats?.todayRevenue ?? 0).toLocaleString()}`
          }
        ...
        <StatCard
          label="Held Orders"
          value={loading ? "—" : stats!.heldOrders}
      ...
      <div className="dash-card grid grid-cols-2 gap-4 mb-8">
        <div className="bg-white border border-black/[0.07] rounded-2xl p-5">
          <p className="text-xs text-[#1A1714]/40 font-label mb-2">
            This Month Revenue
          </p>
          <p className="text-3xl font-bold text-[#1A1714] font-headline">
      ...
      <div className="dash-card bg-white border border-black/[0.07] rounded-2xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-black/[0.06]">
          <h3 className="text-sm font-bold text-[#1A1714] font-label">
            Recent Orders
          </h3>
          <Link
            href="/admin/orders"
            className="text-xs text-primary hover:text-primary/70 transition-colors font-label flex items-center gap-1"
          >
        ...
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            <p className="text-sm text-[#1A1714]/30 font-label">
              No orders yet
            </p>
          </div>
        ) : (
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-[#1A1714] font-label">
                      {order.order_number}
                    </span>
                    <StatusBadge status={order.status} />
                  </div>
                  <p className="text-xs text-[#1A1714]/40 font-label mt-0.5">
                    {order.customer_name} · {order.district}
                  </p>
                </div>
                <span className="text-sm font-bold text-[#1A1714]/70 font-label shrink-0">
